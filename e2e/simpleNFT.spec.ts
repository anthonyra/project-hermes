...
describe("Simple NFT scenario", () => {
  ...
  it("Users should mint NFT", async () => {
    ...
    const toUser1Response = await client.assets.submitTransaction(
      "MintToken",
      user1MintDto.signed(user1.privateKey, false)
    );
    const toUser2Response = await client.assets.submitTransaction(
      "MintToken",
      user2MintDto.signed(user2.privateKey, false)
    );
  ...
  it("transfer NFT between users", async () => {
    ...
    expect(await fetchNFTInstances(client, nftClassKey, user1.identityKey)).toEqual([]);
    expect(await fetchNFTInstances(client, nftClassKey, user2.identityKey)).toEqual([
      new BigNumber(1),
      new BigNumber(2)
    ]);
  });
